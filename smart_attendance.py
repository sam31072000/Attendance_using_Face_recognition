...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'shaksham31072000@gmail.com' ")
...
def save_pass():
    assure_path_exists("TrainingData/")
    exists1 = os.path.isfile("TrainingData\psd.txt")
    if exists1:
        tf = open("TrainingData\psd.txt", "r")
        key = tf.read()
    else:
        master.destroy()
        ...
        if new_pas == None:
            ...
            tf = open("TrainingData\psd.txt", "w")
            tf.write(new_pas)
    ...
    if (op == key):
        if(newp == nnewp):
            txf = open("TrainingData\psd.txt", "w")
            txf.write(newp)
        else:
...
def TakeImages():
    ...
    serial = 0
    exists = os.path.isfile("at1.csv")
    if exists:
        with open("at1\at1.csv", 'r') as csvFile1:
        ...
        with open("StudentDetails\at1 csvFile1:
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        row = [serial, '', Id, '', name]
        with open('at1.csv', 'a+') as csvFile:
...
def TrackImages():
    ...
    exists3 = os.path.isfile("TrainingData\Trainner.yml")
    if exists3:
    ...
    exists1 = os.path.isfile("at1.csv")
    if exists1:
        df = pd.read_csv("at1.csv")
    else:
...
exists = os.path.isfile("at1.csv")
if exists:
    with open("at1.csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
        for l in reader1:
            res = res + 1
    res = (res // 2) - 1
